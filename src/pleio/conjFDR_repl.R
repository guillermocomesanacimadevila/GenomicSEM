...
dat <- dat[!is.na(SNP) & !is.na(CHR) & !is.na(POS)]
...
dat[, (p1) := as.numeric(get(p1))]
dat[, (p2) := as.numeric(get(p2))]
dat[, (b1) := as.numeric(get(b1))]
dat[, (b2) := as.numeric(get(b2))]
dat <- dat[!is.na(get(p1)) & !is.na(get(p2)) & !is.na(get(b1)) & !is.na(get(b2))]

z_from_p <- function(p, beta) qnorm(p/2, lower.tail = FALSE) * sign(beta)
...
if (nrow(shared) > 0) {
  ...
  cmd <- sprintf(
    '%s --bfile %s --clump %s --clump-field P --clump-p1 1 --clump-p2 1 --clump-r2 0.1 --clump-kb 250 --out %s',
  ...
  if (file.exists(clumped_path)) {
    ...
    fwrite(shared_leads[direction == "Concordant"], file.path(OUT_DIR, paste0(out_prefix, "_shared_leads_concordant.tsv")), sep = "\t")
    fwrite(shared_leads[direction == "Discordant"], file.path(OUT_DIR, paste0(out_prefix, "_shared_leads_discordant.tsv")), sep = "\t")
  }
}
