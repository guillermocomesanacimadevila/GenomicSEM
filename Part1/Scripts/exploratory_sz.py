...
def vcf_to_tsv(vcf_file_path: str, tsv_file_path: str) -> str:
    with open(vcf_file_path, "r") as vcf_in, open(tsv_file_path, "w", newline="") as tsv_out:
        ...
        for raw in vcf_in:
            ...
            if s.startswith("##"):
                continue
            if s.startswith("#"):
                writer.writerow(s.lstrip("#").split("\t"))
            else:
...
def qc_checks(df: pd.DataFrame, drop_indels: bool = False) -> pd.DataFrame:
    print("===== QC Summary =====")
    print(f"Number of rows (variants): {df.shape[0]:,}")
    ...
    if "beta" in cols_lower:
        print("\nEffect size column detected: BETA")
    elif "or" in cols_lower or "lnor" in cols_lower:
        print("\nEffect size column detected: OR/ln(OR)")
    else:
        print("\nNo obvious effect size column (Beta/OR) detected")
    pcol = None
    for candidate in ["P", "PVAL", "p", "pval"]:
    ...
    allele_pairs = [("Effect", "Non_Effect"), ("A1", "A2"), ("EA", "NEA"), ("ALT", "REF"), ("ALLELE1", "ALLELE2")]
    a1, a2 = None, None
    ...
    if a1 and a2:
        ...
        print("\nINDEL check skipped: no allele column pair found")
    print("=======================")
    return df

def harmonise_sz_to_ad(df: pd.DataFrame) -> pd.DataFrame:
    d = df.copy()
    rename_map = {
        "CHROM": "CHR",
        "POS": "BP",
    ...
    present_map = {k: v for k, v in rename_map.items() if k in d.columns}
    d = d.rename(columns=present_map)
    required = ["CHR", "BP", "SNP", "Effect", "Non_Effect", "Beta", "SE", "P"]
    for col in required:
        if col not in d.columns:
            d[col] = pd.NA
    for col in ["CHR", "BP", "Beta", "SE", "P", "FCAS", "FCON", "NCAS", "NCON"]:
        if col in d.columns:
    ...
    if {"FCAS", "FCON"}.issubset(d.columns):
        ...
        if {"NCAS", "NCON"}.issubset(d.columns):
            ...
            wsum = ncas + ncon
            eaf_weighted = (fcas * ncas + fcon * ncon) / wsum
            eaf_mean = (fcas + fcon) / 2.0
            d["Effect_allele_freq"] = np.where(wsum > 0, eaf_weighted, eaf_mean)
        else:
            d["Effect_allele_freq"] = (fcas + fcon) / 2.0
    else:
        d["Effect_allele_freq"] = pd.NA
    chr_str = d["CHR"].astype("Int64").astype(str)
    bp_str = d["BP"].astype("Int64").astype(str)
    ne = d["Non_Effect"].astype(str)
    ef = d["Effect"].astype(str)
    d["Test_MarkerName"] = chr_str + ":" + bp_str + ":" + ne + "_" + ef
    final_cols = ["CHR", "BP", "SNP", "Effect", "Non_Effect", "Beta", "SE", "P", "Effect_allele_freq", "Test_MarkerName"]
    extras = [c for c in d.columns if c not in final_cols]
    d = d[final_cols + extras]
    return d
...
def prepare_for_plots(df: pd.DataFrame):
    ...
    ambiguous = (is_snp & (((eff == "A") & (non == "T")) | ((eff == "T") & (non == "A")) | ((eff == "C") & (non == "G")) | ((eff == "G") & (non == "C"))))
    ...
    fallback = df_out["CHR_int"].astype(str) + ":" + df_out["BP_int"].astype(str) + ":" + non + "_" + eff
    df_out["variant_id"] = np.where(rs_mask, snp_id, np.where(df_out["Test_MarkerName"].notna(), df_out["Test_MarkerName"].astype(str), fallback))
...
def manhattan_plot(
    ...
    *,
    ...
    base_colors = np.array(["#4C78A8", "#9EB7D4"])
    ...
    ax.scatter(d["pos_cum"], d["minus_log10p"], s=point_size, alpha=alpha, c=colors, linewidths=0, rasterized=True)
    if is_high.any():
        ax.scatter(d.loc[is_high, "pos_cum"], d.loc[is_high, "minus_log10p"], s=max(point_size * 2, 6), alpha=1.0, c="#D62728", edgecolor="white", linewidths=0.25, rasterized=True, zorder=3)
    def _hline(yval, ls, col, label):
        ax.axhline(yval, linestyle=ls, linewidth=1.0, color=col, alpha=0.9)
        ax.text(x=ax.get_xlim()[1], y=yval, s=label, va="bottom", ha="right", fontsize=font_size_tick, color=col, bbox=dict(boxstyle="round,pad=0.2", fc="white", ec="none", alpha=0.6))
    if sug_line:
    ...
    if annotate_top_n or annotate_threshold:
        ...
        if "variant_id" in cand.columns:
            for _, row in cand.iterrows():
                ax.annotate(text=str(row["variant_id"]), xy=(float(row["pos_cum"]), float(row["minus_log10p"])), xytext=(0, 6), textcoords="offset points", fontsize=max(font_size_tick - 1, 8), ha="center", va="bottom", color="#111111", clip_on=True)
    fig.tight_layout()
    if out_path:
        for ext in out_formats:
            fig.savefig(f"{out_path}.{ext}", dpi=dpi, bbox_inches="tight", pad_inches=0.05, transparent=False)
    return fig, ax


if __name__ == "__main__":
    vcf_in = "/Users/guillermocomesanacimadevila/Desktop/PhD/Part1/Data/SZ/PGC3_SCZ_wave3.primary.autosome.public.v3.vcf"
    tsv_out_raw = "/Users/guillermocomesanacimadevila/Desktop/PhD/Part1/Data/SZ/PGC3_SCZ_wave3.cleaned.tsv"
    tsv_out_harmonised = "/Users/guillermocomesanacimadevila/Desktop/PhD/Part1/Data/SZ/PGC3_SCZ_wave3.harmonised_to_AD.tsv"
    out_path = vcf_to_tsv(vcf_in, tsv_out_raw)
    print(f"[1/4] Saved cleaned TSV: {out_path}")
    df_sz = pd.read_csv(out_path, sep="\t", dtype=str, low_memory=False)
    df_sz = qc_checks(df_sz, drop_indels=True)
    print(f"[2/4] Completed QC checks on {out_path}")
    df_sz_h = harmonise_sz_to_ad(df_sz)
    df_sz_h.to_csv(tsv_out_harmonised, sep="\t", index=False)
    print(f"[3/4] Saved harmonised TSV: {tsv_out_harmonised}")
    df_prep, xticks, xlabels, meta = prepare_for_plots(df_sz_h)
    print("Prepared rows:", df_prep.shape[0], "| Dropped:", meta["n_dropped"])
    print("Chromosomes detected:", meta["chrom_order"])
    base_no_ext, _ = os.path.splitext(tsv_out_harmonised)
    prepared_tsv_path = base_no_ext + ".prepared_for_plots.tsv"
    df_prep.to_csv(prepared_tsv_path, sep="\t", index=False)
    print(f"Prepared-for-plots TSV: {prepared_tsv_path}")
    manhattan_plot(
        df_prep, xticks, xlabels,
        genomewide=5e-8, suggestive=1e-5,
        snp_only=True, drop_ambiguous=False,
        title="PGC3 SZ (SNP-only)",
        point_size=0.6,
        out_path="manhattan_SZ",
        out_formats=("png", "pdf"),
        annotate_top_n=10,
