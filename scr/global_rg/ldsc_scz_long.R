...
in_files <- c(
  ...
  file.path(data_dir, "AGE/post-qc/timmers2020_healthspan_lifespan_longevity_neff.tsv")
)

trait_names <- c("SCZ", "AGE")
N_vec <- c(SCZ=157013, AGE=709709)
...
age_cases <- 354854; age_controls <- 354855
sample_prev <- c(
  SCZ = scz_cases / (scz_cases+scz_controls),
  AGE = age_cases / (age_cases+age_controls)
)

population_prev <- c(SCZ = 0.01, AGE = 0.10)
ldsc_out <- ldsc(
  traits=out_files,
...
qc_out <- rbindlist(list(
  ...
  data.table(
    Trait = "AGE",
    h2 = as.numeric(uni_AGE$S[1,1]),
    h2_se = sqrt(as.numeric(uni_AGE$V[1,1])),
    mean_chisq = get_mean(uni_AGE),
    Intercept = as.numeric(uni_AGE$I[1,1])
  )
))
...
outdir <- file.path(proj,"outputs/ldsc/scz_age"); dir.create(outdir,showWarnings=FALSE,recursive=TRUE)
fwrite(qc_out,file.path(outdir,"ldsc_trait_qc_SCZ_AGE.tsv"),sep="\t")
...
write.csv(global, file.path(proj, "outputs/ldsc/scz_age_ldsc_results.tsv"), sep="\t")
I_mat <- ldsc_out$I
rownames(I_mat) <- colnames(I_mat) <- c("SCZ","AGE")
overlap_corr <- I_mat / sqrt(outer(diag(I_mat), diag(I_mat), "*"))
diag(overlap_corr) <- 1
write.csv(overlap_corr, file.path(outdir, "overlap_corr_for_LAVA_SCZ_AGE.csv"), quote=FALSE)
print(overlap_corr)
