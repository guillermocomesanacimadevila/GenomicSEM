...
population_prev <- c(AD=0.07, BIP=0.01)
ldsc_out <- ldsc(
  traits=out_files,
...
outdir <- file.path(proj,"outputs/ldsc/ad_bip"); dir.create(outdir,showWarnings=FALSE,recursive=TRUE)
fwrite(qc_out,file.path(outdir,"ldsc_trait_qc_AD_BIP.tsv"),sep="\t")
...
I_mat <- ldsc_out$I
rownames(I_mat) <- colnames(I_mat) <- c("AD","BIP")
overlap_corr <- I_mat / sqrt(outer(diag(I_mat), diag(I_mat), "*"))
diag(overlap_corr) <- 1
write.csv(overlap_corr, file.path(outdir, "overlap_corr_for_LAVA_AD_BIP.csv"), quote=FALSE)
print(overlap_corr)
