...
trait_names <- c("AD", "AGE")
N_vec <- c(AD=88394, AGE=709709)
...
population_prev <- c(AD = 0.07, AGE = 0.10)
ldsc_out <- ldsc(
  traits=out_files,
...
outdir <- file.path(proj,"outputs/ldsc/ad_age"); dir.create(outdir,showWarnings=FALSE,recursive=TRUE)
fwrite(qc_out,file.path(outdir,"ldsc_trait_qc_AD_AGE.tsv"),sep="\t")
...
write.csv(global, file.path(proj, "outputs/ldsc/ad_age_ldsc_results.tsv"), sep="\t")
I_mat <- ldsc_out$I
rownames(I_mat) <- colnames(I_mat) <- c("AD","AGE")
overlap_corr <- I_mat / sqrt(outer(diag(I_mat), diag(I_mat), "*"))
diag(overlap_corr) <- 1
write.csv(overlap_corr, file.path(outdir, "overlap_corr_for_LAVA_AD_AGE.csv"), quote=FALSE)
print(overlap_corr)
