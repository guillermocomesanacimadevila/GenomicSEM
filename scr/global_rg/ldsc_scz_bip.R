...
trait_names <- c("SCZ", "BIP")
N_vec <- c(SCZ=157013, BIP=492428)
...
bip_cases <- 130064; bip_controls <- 2301519
sample_prev <- c(
  SCZ = scz_cases / (scz_cases+scz_controls),
  BIP = bip_cases / (bip_cases+bip_controls)
)

population_prev <- c(SCZ=0.01, BIP=0.01)
ldsc_out <- ldsc(
  traits=out_files,
...
qc_out <- rbindlist(list(
  ...
  data.table(
    Trait = "BIP",
    h2 = as.numeric(uni_BIP$S[1,1]),
    h2_se = sqrt(as.numeric(uni_BIP$V[1,1])),
    mean_chisq = get_mean(uni_BIP),
    Intercept = as.numeric(uni_BIP$I[1,1])
  )
))
...
outdir <- file.path(proj,"outputs/ldsc/scz_bip"); dir.create(outdir,showWarnings=FALSE,recursive=TRUE)
fwrite(qc_out,file.path(outdir,"ldsc_trait_qc_SCZ_BIP.tsv"),sep="\t")
...
I_mat <- ldsc_out$I
rownames(I_mat) <- colnames(I_mat) <- c("SCZ","BIP")
overlap_corr <- I_mat / sqrt(outer(diag(I_mat), diag(I_mat), "*"))
diag(overlap_corr) <- 1
write.csv(overlap_corr, file.path(outdir, "overlap_corr_for_LAVA_SCZ_BIP.csv"), quote=FALSE)
print(overlap_corr)